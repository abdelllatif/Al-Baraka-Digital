...
public class OperationServiceImpl implements OperationService {
    ...
    public Operation createOperation(Long clientId, OperationRequest request) {
        ...
        switch (request.getType()) {
            case DEPOSIT -> handleDeposit(request, operation, client);
            case WITHDRAWAL -> handleWithdrawal(request, operation, client);
            case TRANSFER -> handleTransfer(request, operation, client);
    ...
    private void handleDeposit(OperationRequest request, Operation operation, User client) {
        ...
        operation.setAccountSource(account); // فقط accountSource
    }
    ...
    private void handleTransfer(OperationRequest request, Operation operation, User client) {
        ...
        if (request.getAmount().compareTo(THRESHOLD_AMOUNT) <= 0 &&
                request.getAmount().compareTo(sourceAccount.getBalance()) > 0) {
            throw new IllegalArgumentException("Insufficient balance");
        }
    ...
    public List<Operation> getClientOperations(Long clientId) {
        ...
        return sourceOps.stream().distinct().toList();
    }
    ...
    private void executeOperation(Operation operation) {
        if (operation.getStatus() != OperationStatus.APPROVED) return;

        switch (operation.getType()) {
            case DEPOSIT -> executeDeposit(operation);
            case WITHDRAWAL -> executeWithdrawal(operation);
            case TRANSFER -> executeTransfer(operation);
        }
